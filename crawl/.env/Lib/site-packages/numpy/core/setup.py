...
def check_svml_submodule(svmlpath):
    if not os.path.exists(svmlpath + "/README.md"):
        raise RuntimeError(
            "Missing `SVML` submodule! Run `git submodule "
...
def check_math_capabilities(config, ext, moredefs, mathlibs):
    ...
    if not check_funcs_once(MANDATORY_FUNCS):
        raise SystemError(
            "One of the required function to build numpy is not"
...
def check_mathlib(config_cmd):
    ...
    for libs in mathlibs_choices:
        if config_cmd.check_func(
            "log", libraries=libs, call_args="0", decl="double log(double);", call=True
        ):
            mathlibs = libs
            break
    else:
...
def configuration(parent_package="", top_path=None):
    ...
    config.add_extension(
        "_rational_tests", sources=[join("src", "umath", "_rational_tests.c")]
    )
    ...
    config.add_extension(
        "_struct_ufunc_tests", sources=[join("src", "umath", "_struct_ufunc_tests.c")]
    )
    ...
    config.add_extension(
        "_operand_flag_tests", sources=[join("src", "umath", "_operand_flag_tests.c")]
    )
